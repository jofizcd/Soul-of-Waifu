...
class ConfigurationCharacters():
    ...
    def save_characterai_to_list(self, character_name, character_id, chat_id, character_avatar, character_title, character_description, character_personality, first_message, voice_name, character_ai_voice_id, elevenlabs_voice_id, voice_type, rvc_enabled, rvc_file, expression_images_folder, live2d_model_folder, conversation_method):
        """
    ...
    def save_character_card(self, character_name, character_title, character_avatar, character_description, character_personality, first_message, elevenlabs_voice_id, voice_type, rvc_enabled, rvc_file, expression_images_folder, live2d_model_folder, conversation_method):
        """
        ...
        Args:
            ...
            rvc_enabled (bool): RVC enabled for character,
            rvc_file (str): RVC file path, 
            conversation_method (str): Character's conversation method.
        """
        configuration_data = self.load_configuration()
        if 'character_list' not in configuration_data:
    ...
    def add_message_to_chat_content(self, character_name, author_name, is_user, text):
        ...
        for existing_message in chat_content.values():
            if existing_message["is_user"] == is_user and existing_message["text"] == text:
                print("Message already exists in chat_content. Skipping.")
                return
    ...
    def add_message_to_chat_content_regenerate(self, character_name, author_name, is_user, text):
        configuration_data = self.load_configuration()
        if 'character_list' not in configuration_data:
            raise ValueError("Character list not found in configuration data.")
        ...
        for message_id, existing_message in chat_content.items():
            if (
                existing_message["author_name"] == author_name and
                existing_message["is_user"] == is_user
            ):
                print(f"Message already exists with ID {str(int(message_id) + 2)}. Updating it.")
                chat_content[str(int(message_id) + 2)]["text"] = text
                break
        else:
            new_message_id = str(max((int(k) for k in chat_content.keys()), default=0) + 1)
            chat_content[new_message_id] = {
