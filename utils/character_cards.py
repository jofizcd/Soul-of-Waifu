...
class CharactersCard():
    ...
    async def fetch_trending_character_data(self):
        url = "https://gateway.chub.ai/search?first=50&page=1&namespace=characters&nsfw=false&nsfw_only=false&nsfl=false&min_tokens=50&max_tokens=100000&chub=true&sort=trending&venus=true&count=false"
    ...
    async def search_character(self, character_name):
        url = f"https://gateway.chub.ai/search?excludetopics=NSFW&first=50&page=1&namespace=characters&search={character_name}&include_forks=true&nsfw=true&nsfw_only=false&nsfl=false&asc=false&min_ai_rating=0&min_tokens=50&max_tokens=100000&chub=true&exclude_mine=true&sort=default&topics=&inclusive_or=false&recommended_verified=false&venus=true&count=false"
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                if response.status == 200: 
                    try:
