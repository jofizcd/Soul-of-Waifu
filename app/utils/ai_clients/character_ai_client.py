...
logger = logging.getLogger("Character AI Client")
...
class CharacterAI():
    ...
    async def create_character(self, character_id):
        ...
        for message in all_messages:
            try:
                ...
                chat_content[message_id] = {
                    ...
                    "is_user": author_name != character_name,
                    "text": text
                }
        ...
        try:
            ...
            if character_name not in character_list:
                ...
                character_list[character_name].update({
                    "chat_content": chat_content
                })
            ...
            logger.error(f"Error working with config: {e}")
    ...
    async def fetch_chat(self, chat_id, character_name, character_id, character_avatar_url, character_title, character_description, character_personality, first_message, current_text_to_speech, voice_name, character_ai_voice_id, elevenlabs_voice_id, voice_type, rvc_enabled, rvc_file, current_sow_system_mode, expression_images_folder, live2d_model_folder, vrm_model_file, conversation_method, current_emotion):
        ...
        while True:
            try:
                ...
                logger.error(f"Error when receiving messages: {e}")
                break

        all_messages.reverse()

        character_data = self.configuration_characters.load_configuration()
        chat_config = character_data["character_list"].get(character_name, {})
        chat_content = chat_config.get("chat_content", {})
        ...
        for message in all_messages:
            try:
                ...
                existing_msg = next(
                    (msg for msg in chat_content.items() if msg[1].get("text") == text),
                    None
                )

                if existing_msg:
                    message_id = existing_msg[0]
                else:
                    message_id = str(uuid.uuid4())
                
                new_chat_content[message_id] = {
                    "sequence_number": sequence_number,
                    "turn_id": turn_id,
                    "candidate_id": primary_candidate_id,
                    "author_name": author_name,
        ...
        try:
            ...
            logger.error(f"Error when working with the configuration: {e}")
    ...
    async def play_preview_voice(self, voice_id):
        ...
        filename = "app/cache/character_voice_preview.wav"
        ...
        if await self.download_file(voice_preview_url, filename):
            if os.path.exists(filename) and os.path.getsize(filename) > 0:
                ...
                logger.error("Downloaded file is empty or corrupt.")
        else:
            logger.error("Failed to download the preview file.")
    ...
    async def create_new_chat(self, character_id):
        ...
        voice_type = character_list[character_name]["voice_type"]
        rvc_enabled = character_list[character_name]["rvc_enabled"]
        rvc_file = character_list[character_name]["rvc_file"]
        ...
        vrm_model_file = character_list[character_name]["vrm_model_file"]
        character_avatar = character_information.avatar
        character_avatar_url = character_avatar.get_url()
        chat_id = chat.chat_id
        current_emotion = character_list[character_name]["current_emotion"]
        
        await self.fetch_chat(chat_id, character_name, character_id, character_avatar_url, character_title, character_description, character_personality, greeting_message, current_text_to_speech, voice_name, character_ai_voice_id, elevenlabs_voice_id, voice_type, rvc_enabled, rvc_file, current_sow_system_mode, expression_images_folder, live2d_model_folder, vrm_model_file, conversation_method="Character AI", current_emotion=current_emotion)
    ...
    async def edit_message(self, character_name, message_id, chat_id, turn_id, candidate_id, text):
        ...
        try:
            ...
            if character_name not in character_list:
                logger.error(f"The character '{character_name}' was not found in the configuration.")
                return

            chat_content = character_list[character_name].get("chat_content", {})
            if message_id not in chat_content:
                logger.error(f"The message with the ID {message_id} was not found in chat_content.")
                return
            ...
            chat_content[message_id]["text"] = edited_text
            chat_content[message_id]["author_name"] = author_name
            chat_content[message_id]["candidate_id"] = primary_candidate_id
            ...
            logger.error(f"Error editing the message: {e}")
            import traceback
            traceback.print_exc()
    ...
    async def delete_message(self, character_name, message_id, chat_id, turn_id):
        ...
        if message_id in chat_content:
            ...
            configuration_data["character_list"] = character_list
            self.configuration_characters.save_configuration_edit(configuration_data)
            self.configuration_characters.renumber_sequence_numbers(character_name)
    ...
    async def send_message(self, character_id, chat_id, user_message, streaming=True):
        await self.get_client()
        answer = await self.client.chat.send_message(character_id, chat_id, user_message, streaming)

        async for message in answer:
            yield message
    
    async def regenerate_message(self, character_id, chat_id, turn_id, streaming=True):
        await self.get_client()
        regenerated_answer = await self.client.chat.another_response(character_id, chat_id, turn_id, streaming)

        async for message in regenerated_answer:
            yield message
